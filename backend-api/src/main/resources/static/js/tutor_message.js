(function () {
  ...
  async function loadConversations() {
    ...
    // Check query param for auto-open
    const params = new URLSearchParams(window.location.search);
    const cid = params.get('conversationId');
    if (cid && !state.convoId) {
      const found = convos.find(c => String(c.conversationId) === String(cid));
      if (found) {
        selectConversation(found.conversationId, found.otherName);
      }
    }
  }
